...
bool run_query(sqlite3* db, const std::string& sql, std::vector<user_record>& records)
{
    ...
    std::regex pattern("or\\s+(['\"]?[\\w]+['\"]?)\\s*=\\s*\\1|or\\s+[0-9]+\\s*=\\s*\\d+", std::regex::icase);
    std::smatch match;

    // Check if the sql query argument matches the regex pattern for a specific type of sql injection
    if (std::regex_search(localCopy, match, pattern)) {
        std::cout << "SQL Injection Attack Suspected: " << sql << std::endl;
        return false;
    }
